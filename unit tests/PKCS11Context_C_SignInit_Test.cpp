...
TEST(PKCS11Context, C_SignInit_InvalidSession) {
  MockCardManager manager;
  PKCS11Context context(&manager);

  ASSERT_EQ(CKR_SESSION_HANDLE_INVALID, context.C_SignInit(101, NULL, NULL_PTR));
}

TEST(PKCS11Context, C_SignInit_UserNotLoggedIn) {
  MockCardManager manager;
  ...
  ASSERT_EQ(CKR_USER_NOT_LOGGED_IN, context.C_SignInit(session, NULL, NULL_PTR));
  ASSERT_EQ(CKR_OK, context.C_CloseSession(session));

  delete cert;
}

TEST(PKCS11Context, C_SignInit_InvalidMechanism) {
  MockCardManager manager;
  EXPECT_CALL(manager, getTokenCount(true)).WillRepeatedly(Return(1));
  EXPECT_CALL(manager, isInReader(_)).WillRepeatedly(Return(true));
  ByteVec *cert = getCert();
  ...
  ASSERT_EQ(CKR_MECHANISM_INVALID, context.C_SignInit(session, &mechanism, NULL_PTR));
  ASSERT_EQ(CKR_OK, context.C_Logout(session));
  ASSERT_EQ(CKR_OK, context.C_CloseSession(session));
}

TEST(PKCS11Context, C_SignInit) {
  MockCardManager manager;
  EXPECT_CALL(manager, getTokenCount(true)).WillRepeatedly(Return(1));
  EXPECT_CALL(manager, isInReader(_)).WillRepeatedly(Return(true));
  ByteVec *cert = getCert();
